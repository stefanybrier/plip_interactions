...
class PiStacking(Interaction):
    ...
    def __str__(self):
        return super().__str__() + f" | Distance: {self.DIST} | Angle: {self.ANGLE} | Offset: {self.OFFSET}"

class WaterBridge(Interaction):
    def __init__(self, parts):
        super().__init__(parts)
        self.DIST_H_O = parts[6]
        self.DIST_O_A = parts[7]
        self.WATER_IDX = parts[8]
    
    def __str__(self):
        return super().__str__() + f" | Distance H-O: {self.DIST_H_O} | Distance O-A: {self.DIST_O_A}"
...
class MetalComplex(Interaction):
    def __init__(self, parts):
        ...
        self.DIST = parts[6]
        self.GEOM = parts[7]
        self.LIG_IDX_LIST = parts[8]
    
    def __str__(self):
        return super().__str__() + f" | Distance: {self.DIST} | Geometry: {self.GEOM}"
...
section_parsers = {
    ...
    "pi-Stacking": ("pi-Stacking:", PiStacking),
    "Water Bridges": ("Water Bridge:", WaterBridge),
    "Metal Complexes": ("Metal Complex:", MetalComplex)
}
